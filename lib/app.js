...
import { formatLocation, formatYelp } from './utils.js';
...
app.get('/yelp', async (req, res) => {
  try {
    const apiKey = process.env.LOCATION_IQ_API_KEY;
    const response = await request.get(`https://api.yelp.com/v3/businesses/search?latitude=${req.query.latitude}&longitude=${req.query.longitude}`).set('Authorization', `Bearer ${apiKey}}`);
